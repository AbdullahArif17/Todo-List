...
let main = async () => {
  while (condition) {
    ...
    if (option.options === "Add Task") {
      ...
      condition = false;
      console.log(
        chalk.blueBright(`Your Todo-List: ${chalk.whiteBright.bold(todos)}`)
      );
    }
  }
};
...
let deleteTask = async () => {
  ...
  let taskIndex = await inquirer.prompt([
    {
      ...
      message: chalk.blueBright(
        "Enter the Index No. of task you want to delete from your list:"
      ),
    },
  ]);
  ...
  console.log(
    chalk.greenBright(
      `\n ${chalk.green(
        deletedTask
      )} this task is deleted from your Todo-List\n`
    )
  );
};
